...
GNetSnmpEnum const if_mib_enums_ifTestStatus[] = {
    { IF_MIB_IFTESTSTATUS_NOTINUSE, "notInUse" },
    { IF_MIB_IFTESTSTATUS_INUSE,    "inUse" },
    { 0, NULL }
};

GNetSnmpEnum const if_mib_enums_ifTestResult[] = {
...
static gint32 ifTestId_constraints[] = {0L, 2147483647L, 0, 0};
static guint16 ifTestOwner_constraints[] = {0U, 255U, 0, 0};
...
static GNetSnmpAttribute ifTestEntry_attr[] = {
    { 1, GNET_SNMP_VARBIND_TYPE_INTEGER32,
...
static inline int
unpack_ifTestEntry(GNetSnmpVarBind *vb, if_mib_ifTestEntry_t *ifTestEntry)
{
    guint8 idx = 11;

    if (vb->oid_len < idx) return -1;
    ifTestEntry->ifIndex = vb->oid[idx++];
    if ((ifTestEntry->ifIndex < 1)) {
         return -1;
    }
    if (vb->oid_len > idx) return -1;
    return 0;
...
static inline if_mib_ifTestEntry_t *
assign_ifTestEntry(GList *vbl)
{
    if_mib_ifTestEntry_t *ifTestEntry;
    char *p;
    ...
    * (GList **) p = vbl;
    ...
    if (unpack_ifTestEntry((GNetSnmpVarBind *) vbl->data, ifTestEntry) < 0) {
        g_warning("%s: invalid instance identifier", "ifTestEntry");
        g_free(ifTestEntry);
        return NULL;
    }

    gnet_snmp_attr_assign(vbl, ifTestEntry_oid, G_N_ELEMENTS(ifTestEntry_oid),
...
void
if_mib_get_ifTestTable(GNetSnmp *s, if_mib_ifTestEntry_t ***ifTestEntry, gint64 mask, GError **error)
{
    GList *in = NULL, *out = NULL;
    GList *row;
    int i;
    static guint32 const _base[] = {1, 3, 6, 1, 2, 1, 31, 1, 3, 1, 0};
    guint32 base[128];

    *ifTestEntry = NULL;
    memcpy(base, _base, sizeof(_base));

    gnet_snmp_attr_get(s, &in, base, 11, 10, ifTestEntry_attr, mask);

    out = gnet_snmp_sync_table(s, in, error);
    /* gnet_snmp_varbind_list_free(in); */

    if (out) {
        *ifTestEntry = (if_mib_ifTestEntry_t **) g_malloc0((g_list_length(out) + 1) * sizeof(if_mib_ifTestEntry_t *));
        for (row = out, i = 0; row; row = g_list_next(row), i++) {
...
void
if_mib_get_ifTestEntry(GNetSnmp *s, if_mib_ifTestEntry_t **ifTestEntry, gint32 ifIndex, gint64 mask, GError **error)
{
    GList *in = NULL, *out = NULL;
    guint32 base[128];
    gint8 len;
    ...
    *ifTestEntry = NULL;
    ...
    gnet_snmp_attr_get(s, &in, base, len, 10, ifTestEntry_attr, mask);
...
void
if_mib_set_ifTestEntry(GNetSnmp *s, if_mib_ifTestEntry_t *ifTestEntry, gint64 mask, GError **error)
{
    GList *in = NULL, *out = NULL;
    guint32 base[128];
    gint8 len;
    ...
    gnet_snmp_attr_set(s, &in, base, len, 10, ifTestEntry_attr, mask, ifTestEntry);
